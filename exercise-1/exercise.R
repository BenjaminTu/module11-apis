...
## Then send GET() request to fetch the data, then extract the answer to the question
	base <- "http://data.unhcr.org/api/"
# For what years does the API have statistical data?
	info <- fromJSON(content(GET(paste(base, "stats/time_series_years.json", sep = "")), "text"))
	print(info)
# What is the "country code" for the "Syrian Arab Republic"?
	info <- fromJSON(content(GET(paste(base, "countries/list.json", sep = "")), "text"))
	print(info[["Syrian Arab Republic"]])
